...
@click.option(
    "--skip-shapley", is_flag=True, default=False, help="Skip Shapley (faster — only CF ablation)."
)
def analyze(
    skill_path, eval_path, env_file, target_model, judge_models, permutations, skip_shapley
):
    ...
    def scorer(skill_md_variant: str) -> float:
        ...
        for c in cases:
            ...
            verdict = judges.score(
                input=c.input, response=resp, principle=c.principle, rubric=c.rubric
            )
    ...
    if not split.sections:
        console.print(
            "[yellow]No top-level sections (# / ## / ###) found — nothing to ablate.[/yellow]"
        )
        sys.exit(0)

    console.print(f"[cyan]Found {len(split.sections)} sections. Computing baseline score...[/cyan]")
    baseline = scorer(md)
    ...
    t = Table(title="Counterfactual Ablation (positive = section helps)", box=box.ASCII)
    ...
    if not skip_shapley:
        console.print(
            f"[cyan]Running TMC-Shapley with {permutations} permutations "
            f"(will cache subsets to save cost)...[/cyan]"
        )
        phi = tmc_shapley(md, scorer, num_permutations=permutations, seed=42)

        t2 = Table(title="TMC-Shapley attribution (sum ≈ baseline - empty)", box=box.ASCII)
...
@click.option("--body-limit", default=6000, show_default=True, help="Soft cap on body length.")
...
@click.option(
    "--run-dir", required=True, type=click.Path(), help="Directory to persist SkillRun artifacts."
)
@click.option(
    "--env-file",
    default=None,
...
@click.option(
    "--judge-models",
    default="qwen3.6-plus,qwen-max,qwen-plus",
    show_default=True,
    help="Comma-sep list of judge models (DashScope / Qwen).",
)
def compare(seed_path, challenger_path, eval_path, run_dir, env_file, target_model, judge_models):
    ...
    console.print("[cyan]Evaluating seed...[/cyan]")
    seed_runs = opt._eval_skill(seed_md, cases)
    console.print("[cyan]Evaluating challenger...[/cyan]")
    ...
    from caliper.safety import PairedComparison, hedges_g, paired_bca_bootstrap
    ...
    for c, s, ch in zip(cases, seed_scores, chal_scores, strict=False):
        d = ch - s
        ...
        t.add_row(
            ...
            f"{s:.2f}",
            f"{ch:.2f}",
            f"[{'green' if d > 0 else ('red' if d < 0 else 'white')}]{d:+.2f}[/] {mark}",
        )
    console.print(t)
    ...
    verdict.add_row(
        "BCa sig at 0?", "[green]YES[/green]" if ci.significant_at_zero else "[red]NO[/red]"
    )
    ...
    verdict.add_row(
        "linter seed HIGH", str(sum(1 for f in seed_lint if f.severity == Severity.HIGH))
    )
    verdict.add_row(
        "linter chal HIGH",
...
@cli.command()
@click.argument("seed_path", type=click.Path(exists=True, dir_okay=False))
@click.option("--eval", "eval_path", required=True, type=click.Path(exists=True, dir_okay=False))
@click.option("--rounds", default=3, show_default=True, help="Max proposal rounds.")
@click.option("--run-dir", required=True, type=click.Path())
@click.option("--env-file", default=None, type=click.Path())
@click.option("--target-model", default="qwen3.6-plus", show_default=True)
@click.option("--judge-models", default="qwen3.6-plus,qwen-max,qwen-plus", show_default=True)
@click.option(
    "--cs-margin",
    default=0.02,
    show_default=True,
...
@click.option(
    "--max-tokens", default=None, type=int, help="Hard total-token budget. None = unlimited."
)
@click.option(
    "--max-cost-cny",
    default=None,
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Resume run_dir: use existing champion.md as new seed, skipping completed rounds.",
)
def iterate(
    seed_path,
    eval_path,
    ...
    for r in result.rounds:
        ...
        summary.add_row(
            ...
            (f"[{s['ci']['ci_lower']:+.2f},{s['ci']['ci_upper']:+.2f}]" if s["ci"] else "-"),
